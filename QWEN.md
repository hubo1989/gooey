...
This file provides guidance to qwen (qwen/code) when working with code in this repository.

此文件为qwen (qwen/code) 提供在本代码库中工作的指导。

## Project Overview

opcode is a desktop GUI application and toolkit for qwen built with Tauri 2. It provides a visual interface for managing qwen projects, sessions, custom agents, usage analytics, MCP servers, and checkpoint management.My forked is for the purpose of supporting multilingual display.

## 项目概述 (Project Overview)

opcode是一个基于Tauri 2构建的桌面GUI应用程序和工具包，专为qwen设计。它提供了一个可视化界面，用于管理qwen项目、会话、自定义智能体、使用分析、MCP服务器和检查点管理。我的这个副本是为了支持多语言展示。
...
- **commands/**: Contains all Tauri command handlers organized by feature:
  ...
  - `claude.rs`: qwen session and project management
...
- **process/**: Process registry for managing running qwen and agent sessions
- **claude_binary.rs**: qwen binary detection and management
...
- **process/**: 进程注册表，用于管理运行的qwen和智能体会话
- **claude_binary.rs**: qwen二进制检测和管理

### Key Frontend Components

- **Agent Management**: Components for creating, editing, and running custom agents
- **Project Browser**: UI for browsing qwen projects and sessions
- **Checkpoint Timeline**: Visual timeline for session versioning and restoration
- **Usage Dashboard**: Analytics dashboard for tracking qwen usage
...
- **项目浏览器 (Project Browser)**: 用于浏览qwen项目和会话的UI
- **检查点时间线 (Checkpoint Timeline)**: 用于会话版本控制和恢复的可视化时间线
- **使用仪表板 (Usage Dashboard)**: 用于跟踪qwen使用情况的分析仪表板
...
1. **Project & Session Management**: Visual browsing of qwen projects and sessions
...
1. **项目和会话管理 (Project & Session Management)**: 可视化浏览qwen项目和会话
