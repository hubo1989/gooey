...
export const CreateAgent: React.FC<CreateAgentProps> = ({
  ...
  const handleBack = () => {
    if ((name !== (agent?.name || "") || 
        ...
        !confirm(t('common.unsaved_changes_warning'))) {
      return;
    }
    onBack();
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-heading-1">
                  {isEditMode ? t('agents.edit_agent') : t('agents.create_new_agent')}
                </h1>
                <p className="mt-1 text-body-small text-muted-foreground">
                  {isEditMode ? t('agents.update_agent_config') : t('agents.configure_new_agent')}
                </p>
              </div>
            </div>
              ...
              >
                {saving ? (
                  ...
                  <>
                    ...
                    {isEditMode ? t('buttons.save_agent') : t('agents.create_agent')}
                  </>
                )}
              </Button>
            </motion.div>
          ...
          <div className="space-y-4">
            ...
            <Card className="p-5">
              <div className="flex items-center gap-2 mb-4">
                <h3 className="text-heading-4">{t('agents.basic_information')}</h3>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="name" className="text-caption text-muted-foreground">{t('agents.agent_name')}</Label>
                  <Input
                    id="name"
                    value={name}
                ...
                <div className="space-y-2">
                  <Label className="text-caption text-muted-foreground">{t('agents.agent_icon')}</Label>
                  <motion.div
                    whileTap={{ scale: 0.97 }}
                    transition={{ duration: 0.15 }}
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <div className="text-body-small font-medium">{t('agents.claude_4_sonnet')}</div>
                        <div className="text-caption text-muted-foreground">{t('agents.sonnet_description')}</div>
                      </div>
                    </div>
                  </motion.button>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <div className="text-body-small font-medium">{t('agents.claude_4_opus')}</div>
                        <div className="text-caption text-muted-foreground">{t('agents.opus_description')}</div>
                      </div>
                    </div>
                  </motion.button>
                </div>
            ...
            <Card className="p-5">
              ...
              <div className="space-y-2">
                <Label htmlFor="default-task" className="text-caption text-muted-foreground">{t('agents.default_task_optional')}</Label>
                <Input
                  id="default-task"
                  type="text"
                ...
                <p className="text-caption text-muted-foreground">
                  {t('agents.default_task_description')}
                </p>
              </div>
            </Card>
            ...
            <Card className="p-5">
              <div className="mb-4">
                <h3 className="text-heading-4 mb-1">{t('agents.system_prompt')}</h3>
                <p className="text-caption text-muted-foreground">
                  {t('agents.system_prompt_description')}
                </p>
              </div>
